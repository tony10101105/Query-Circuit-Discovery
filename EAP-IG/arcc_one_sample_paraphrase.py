...
for i, (clean, corrupted, label) in tqdm(enumerate(dataloader), total=len(dataloader), desc="Processing samples"):
    ...
    attribute(model, g, batch_slice_data, partial(logit_diff, loss=True, mean=True, version=metric_version), method=method, ig_steps=steps, intervention=intervention, file_idx=i)