...
parser.add_argument('--score_matrix_dir', type=str, default='probing_dataset/Query-Circuit-Dataset/score_matrix/arithmetic_add/llama32-1b')
parser.add_argument('--dataset_path', type=str, default='probing_dataset/arithmetic_add_Llama-32-1B.csv')
parser.add_argument('--output_figure', type=str, default='figures/arithmetic_add_complement.pdf')
...
for i, (clean, corrupted, label) in enumerate(tqdm(dataloader, total=len(dataloader), desc="Processing samples", position=0)):
    ...
    for j in tqdm(range(para_data.shape[0]), total=para_data.shape[0], desc="Processing paraphrases", position=1):
        ...
        circuit_performance, circuit_faithfulness, circuit_complement_faithfulness = [], [], []
