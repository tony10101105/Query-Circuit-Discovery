...
dataloader = ds.to_dataloader(batch_size=120)
...
results_g1, all_node_rep_g1, all_layer_rep_g1, _ = evaluate_graph(model, g1, dataloader, partial(prob_diff, loss=False, mean=False), hook_rep=True, hook_layer=True, hook_pattern=True)
results_g1 = results_g1.mean().item()

results_g2, all_node_rep_g2, all_layer_rep_g2, _ = evaluate_graph(model, g2, dataloader, partial(prob_diff, loss=False, mean=False), hook_rep=True, hook_layer=True)
...
print('all_node_rep_g1: ', all_node_rep_g1.shape) # (data_num, n_pos, node_num_g1, 768)
...
all_node_rep_g2 = all_node_rep_g2.permute(2, 0, 1, 3).cpu().numpy() # (node_num_g2, data_num, n_pos, 768)
...
all_node_rep_g1 = np.stack(x_pca)  # Shape: (node_num_g1, data_num, 300)
...
for i in tqdm(range(node_num_g2)):
    ...
    x_pca.append(reduced)
all_node_rep_g2 = np.stack(x_pca)  # Shape: (node_num_g2, data_num, 300)
...
for i in tqdm(range(node_num_g1), desc='Outer'):
    ...
    for j in tqdm(range(node_num_g2), desc='Inner', leave=False):
        ...
        # print('X shape: ', X.shape) # (data_num, 300)
        # print('Y shape: ', Y.shape) # (data_num, 300)

        results = get_representation_similarity(X.T, Y.T, methods=similarity_method)
        feature_distances[i, j] = results


np.savez("preprocessed_data/gt_cost_matrix_all_atthead_rep.npz", 
