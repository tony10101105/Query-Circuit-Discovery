...
for i, (clean, corrupted, label) in enumerate(tqdm(dataloader, total=len(dataloader), desc="Processing samples", position=0)):
    para_data = []
    for k in range(10):
        para_data.append(np.load(f"score_data/arithmetic_mul/{i+300}_{k}.npy"))
    as_tuples = [tuple(arr.flatten()) for arr in para_data]
    num_unique = len(set(as_tuples))
...
plt.tight_layout()
plt.savefig(f'figures/arithmetic_mul_perturb_3.pdf')