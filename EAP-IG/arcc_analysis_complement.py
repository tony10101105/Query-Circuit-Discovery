...
ds = EAPDataset(f'probing_dataset/arc_{category}_Llama-32-1B.csv', num_samples=400)
...
for i, (clean, corrupted, label) in enumerate(tqdm(dataloader, total=len(dataloader), desc="Processing samples", position=0)):
    para_data = []
    for k in range(10):
        para_data.append(np.load(f"score_data//arc_{category}/metric4_{i+800}_{k}.npy"))
...
plt.tight_layout()
plt.savefig(f'figures/arc_{category}_complement_3.pdf')