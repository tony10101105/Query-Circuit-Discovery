...
from preprocessed_circuit.ioi_circuit_constant import IOI_CIRCUIT, head_2_layer, IOI_CIRCUIT_GROUP_A, IOI_CIRCUIT_GROUP_B, GROUP_A_IDX_2_CLASS, GROUP_B_IDX_2_CLASS

from utils import hierarchical_clustering, spectral_clustering, dbscan_clustering
...
for lamb in tqdm(lambda_decay):
    ...
    data = np.load('preprocessed_data/ioi_cost_matrix_all_atthead_rep.npz', allow_pickle=True)
    ...
    data = np.load('preprocessed_data/ioi_cost_matrix_all_atthead_pattern.npz', allow_pickle=True)
    ...
    ### clustering analysis
    all_head_idx_tup = sorted(group_a_idx_tup + group_b_idx_tup)
    all_head_idx = [i[0]*12 + i[1] for i in all_head_idx_tup]
    all_head_2_class = {**GROUP_B_IDX_2_CLASS, **GROUP_A_IDX_2_CLASS}
    node_distances_ab_ab = node_distances[np.ix_(all_head_idx, all_head_idx)]
    # node_clusters = hierarchical_clustering(node_distances_ab_ab, all_head_idx_tup, all_head_idx, all_head_2_class)
    # node_clusters = spectral_clustering(node_distances_ab_ab, all_head_idx_tup, all_head_idx, all_head_2_class)
    node_clusters = dbscan_clustering(node_distances_ab_ab, all_head_idx_tup, all_head_idx, all_head_2_class)
    exit(0)
