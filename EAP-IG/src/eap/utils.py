...
def make_hooks_and_matrices(model: HookedTransformer, graph: Graph, batch_size:int , n_pos:int, scores: Optional[Tensor], hook_rep:bool=False, hook_layer:bool=False, hook_pattern:bool=False):
    ...
    if hook_rep:
        node_representation = torch.zeros((batch_size, n_pos, graph.n_forward, model.cfg.d_model), device=model.cfg.device, dtype=model.cfg.dtype)
    else:
        node_representation = None
    if hook_layer:
        layer_representation = torch.zeros((batch_size, n_pos, graph.cfg['n_layers'], model.cfg.d_model), device=model.cfg.device, dtype=model.cfg.dtype)
    else:
        layer_representation = None
    if hook_pattern:
        node_pattern = torch.zeros((batch_size, graph.n_forward, n_pos, n_pos), device=model.cfg.device, dtype=model.cfg.dtype)
    else:
        node_pattern = None
    ...
    def activation_hook(index, activations, hook, add:bool=True, hook_rep:bool=False):
        """
            index: neuron index
        """
        acts = activations.detach() # node output representation
        try:
            if separate_activations:
                if add:
        ...
        if hook_rep and not add: # only hook the representations of clean inputs
            node_representation[:, :, index] = acts
    ...
    if hook_layer:
        for layer in range(graph.cfg['n_layers']):
            def layer_hook(acts, hook, layer=layer):  # layer must be default arg to avoid late binding
                layer_representation[:, :, layer] = acts.detach()
...
def compute_mean_activations(model: HookedTransformer, graph: Graph, dataloader: DataLoader, per_position=False):
    ...
    for batch in tqdm(dataloader, desc='Computing mean'):
        ...
        if per_position:
            input_lengths = None
        add_to_mean_hooks = [(hook_point, partial(activation_hook, index, means=means, input_lengths=input_lengths, hook_rep=False)) for hook_point, index in hook_points_indices]
