...
for i, (clean, corrupted, label) in enumerate(tqdm(dataloader, total=len(dataloader), desc="Processing samples", position=0)):
    ...
    para_data = []
    for k in range(10):
        para_data.append(np.load(f"Query-Circuit-Dataset/score_data/arithmetic_add/{i}_{k}.npy"))
    ...
    for j in tqdm(range(para_data.shape[0]), total=para_data.shape[0], desc="Processing paraphrases", position=1):
        ...
        for topn in topns:
            ...
            # circuit_complement_performance.append(complement_results)
...
plt.tight_layout()
plt.savefig(f'figures/arithmetic_add.pdf')